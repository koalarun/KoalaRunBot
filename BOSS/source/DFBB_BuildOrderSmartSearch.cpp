    ...
    for (size_t a(0); a < numGoalUnitsBuiltBy.size(); ++a) {
      ...
      if (!actionType.isResourceDepot() && actionType.isBuilding()) {
        ...
        if (numGoalUnitsBuiltBy[actionType.ID()] > 0) {
          ...
          _goal.setGoalMax(actionType, std::min(
                             _initialState.getUnitData().getNumTotal(actionType) + additionalProductionBuildingLimit,
                             (int)numGoalUnitsBuiltBy[actionType.ID()]));
        }
      }
    }
...
void DFBB_BuildOrderSmartSearch::recurseOverStrictDependencies(const ActionType& actionType) {
  ...
  for (size_t a(0); a < recursivePrerequisites.size(); ++a) {
    ...
    if (actionType.isResourceDepot() || actionType.isWorker() || actionType.isSupplyProvider() || actionType.
      isRefinery()) {
      continue;
    }
...
UnitCountType DFBB_BuildOrderSmartSearch::calculateSupplyProvidersRequired() {
  ...
  for (size_t a(0); a < ActionTypes::GetAllActionTypes(getRace()).size(); ++a) {
    ...
    supplyNeeded += std::max(_goal.getGoal(actionType), _initialState.getUnitData().getNumTotal(actionType)) *
      actionType.supplyRequired();
  }

  // set the upper bound on supply based on these values
  UnitCountType supplyFromResourceDepots = _initialState.getUnitData().getNumTotal(resourceDepot) * resourceDepot.
    supplyProvided();
