...
void DFBB_BuildOrderStackSearch::search() {
  ...
  if (_firstSearch) {
    _results.upperBound = _params.initialUpperBound
                            ? _params.initialUpperBound
                            : Tools::GetUpperBound(_params.initialState, _params.goal);
...
void DFBB_BuildOrderStackSearch::generateLegalActions(const GameState& state, ActionSet& legalActions) {
  ...
  if (_params.useSupplyBounding) {
    UnitCountType supplySurplus = state.getUnitData().getMaxSupply() + state.getUnitData().getSupplyInProgress() - state
                                                                                                                   .getUnitData()
                                                                                                                   .getCurrentSupply();
    UnitCountType threshold = (UnitCountType)(ActionTypes::GetSupplyProvider(state.getRace()).supplyProvided() * _params
      .supplyBoundingThreshold);
...
bool DFBB_BuildOrderStackSearch::isTimeOut() {
  return (_params.searchTimeLimit && (_results.nodesExpanded % 200 == 0) && (_searchTimer.getElapsedTimeInMilliSec() >
