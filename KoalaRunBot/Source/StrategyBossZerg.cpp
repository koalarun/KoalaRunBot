...
void StrategyBossZerg::updateArmySizes()
{
	...
	for (const std::pair<BWAPI::UnitType, int> & unitCount : you.getCounts())
	{
		...
		if (!type.isBuilding() && !type.isWorker() && UnitUtil::TypeCanAttackGround(type))
		{
...
// An arbitrary criterion for whether we have "enough" army that we can safely focus on making drones.
...
void StrategyBossZerg::makeUrgentReaction(BuildOrderQueue & queue)
{
	...
	const bool enoughLairTechUnits =
		(_gasUnit == BWAPI::UnitTypes::Zerg_Lurker || _gasUnit == BWAPI::UnitTypes::Zerg_Mutalisk)
			...
			: true;
	if (outOfBook && nLarvas == 0 && nHatches < 15 && nDrones >= 9 && _supplyUsed <= 380 &&
		hatcheriesUnderConstruction <= 2 &&
		minerals >= 300 + 300 * hatcheriesUnderConstruction &&
		...
		(hasSpire || findRemainingBuildTime(BWAPI::UnitTypes::Zerg_Spire) >= 20 * 24))
	{
	...
	if (InformationManager::Instance().enemyHasOverlordHunters())
	{
		...
		int spores = 0;
		MacroLocation spore1 = MacroLocation::Macro;		// this arrangement for ZvT and ZvZ
		MacroLocation spore2 = MacroLocation::Natural;
		bool hasNatural =
			Bases::Instance().myNaturalBase() != nullptr &&
		...
		{
			spore1 = MacroLocation::Natural;		// opposite arrangement for ZvP
			spore2 = MacroLocation::Macro;			// help defend against dark templar
			int enemyAir =
				InformationManager::Instance().getNumUnits(BWAPI::UnitTypes::Protoss_Corsair, _enemy) +
				2 * InformationManager::Instance().getNumUnits(BWAPI::UnitTypes::Protoss_Scout, _enemy);
...
void StrategyBossZerg::checkGroundDefenses(BuildOrderQueue & queue)
{
	...
	if (Bases::Instance().myNaturalBase())
	{
		ourHatchery = Bases::Instance().myNaturalBase()->getDepot();
	...
	{
		ourHatchery = Bases::Instance().myStartingBase()->getDepot();
		if (UnitUtil::IsValidUnit(ourHatchery))
		{
	...
	if (nDrones >= 9)
	{
		...
		const bool makeOne =
			...
			enemyPower > ourPower + 6 * nHatches && !_emergencyGroundDefense && totalSunkens < 5 ||
...
void StrategyBossZerg::produceOtherStuff(int & mineralsLeft, int & gasLeft, bool hasEnoughUnits)
{
	...
	// A. If we have enough economy, get our first gas.
	if (nGas == 0 && nFreeGas > 0 && nDrones >= 9 && hasPool &&
		(nDrones > 12 || gas < 300 || (minerals + 100) / (gas + 100) >= 2) &&
		!isBeingBuilt(BWAPI::UnitTypes::Zerg_Extractor))
	{
		addExtractor = true;
	...
	else if (nDrones >= 50 && nFreeGas > 0 && hasPool &&
		!isBeingBuilt(BWAPI::UnitTypes::Zerg_Extractor))
	{
		addExtractor = true;
	}
	...
	// E. If we have a big mineral excess and enough drones, get more extractors no matter what.
		...
		nDrones > 3 * Bases::Instance().baseCount(_self) + 3 * nGas + 6 &&
...
void StrategyBossZerg::drawStrategyBossInformation()
{
	...
	BWAPI::Broodwar->drawTextScreen(x, y, "%cStrat Boss", kWhite);
	y += 13;
	BWAPI::Broodwar->drawTextScreen(x, y, "%cbases %c%d/%d", kYellow, kCyan, nBases, nBases+nFreeBases);
	y += 10;
	BWAPI::Broodwar->drawTextScreen(x, y, "%cpatches %c%d", kYellow, kCyan, nMineralPatches);
	y += 10;
	BWAPI::Broodwar->drawTextScreen(x, y, "%cgeysers %c%d+%d", kYellow, kCyan, nGas, nFreeGas);
	y += 10;
	BWAPI::Broodwar->drawTextScreen(x, y, "%cdrones%c %d/%d", kYellow, kCyan, nDrones, maxDrones);
	y += 10;
	BWAPI::Broodwar->drawTextScreen(x, y, "%c mins %c%d", kYellow, kCyan, nMineralDrones);
	y += 10;
	BWAPI::Broodwar->drawTextScreen(x, y, "%c gas %c%d", kYellow, kCyan, nGasDrones);
	y += 10;
	BWAPI::Broodwar->drawTextScreen(x, y, "%c react%c +%d", kYellow, kCyan, _extraDronesWanted);
	y += 10;
	BWAPI::Broodwar->drawTextScreen(x, y, "%clarvas %c%d", kYellow, kCyan, nLarvas);
	y += 13;
	if (outOfBook)
	{
		BWAPI::Broodwar->drawTextScreen(x, y, "%ceco %c%.2f %d/%d", kYellow, kCyan, _economyRatio, _economyDrones, 1 + _economyTotal);
		y += 10;
		BWAPI::Broodwar->drawTextScreen(x, y, "%carmy %c%d %d %c%s", kYellow, kCyan, myArmySize, enemyAntigroundArmySize, enoughArmy() ? kGreen : kRed, enoughArmy() ? "ok" : "bad");
		...
		for (int i = 1 + int(TechUnit::None); i < int(TechUnit::Size); ++i)
		{
			...
			BWAPI::Broodwar->drawTextScreen(x, y, "%c%s%c%s %c%d",
				kWhite, available[i] ? "* " : "",
				kOrange, techTargetToString(TechUnit(i)).c_str(),
				kCyan, techScores[i]);
		}
		y += 10;
		BWAPI::Broodwar->drawTextScreen(x, y, "%c%s", kGreen, UnitTypeName(_mineralUnit).c_str());
		y += 10;
		BWAPI::Broodwar->drawTextScreen(x, y, "%c%s", kGreen, UnitTypeName(_gasUnit).c_str());
		if (_auxUnit != BWAPI::UnitTypes::None)
		{
			y += 10;
			BWAPI::Broodwar->drawTextScreen(x, y, "%c%d%c %s", kCyan, _auxUnitCount, kGreen, UnitTypeName(_auxUnit).c_str());
		}
		if (_techTarget != TechUnit::None)
		{
			y += 10;
			BWAPI::Broodwar->drawTextScreen(x, y, "%cplan %c%s", kWhite, kGreen,
				techTargetToString(_techTarget).c_str());
		}
	}
	else
	{
		BWAPI::Broodwar->drawTextScreen(x, y, "%c[book]", kWhite);
	}
	if (_emergencyGroundDefense)
	{
		y += 13;
		BWAPI::Broodwar->drawTextScreen(x, y, "%cEMERGENCY", kRed);
	}
}
