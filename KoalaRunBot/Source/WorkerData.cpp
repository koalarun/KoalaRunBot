...
void WorkerData::AddDepot(const BWAPI::Unit unit) {
  ...
  depots_.insert(unit);
  depot_2_worker_count_[unit] = 0;
}

void WorkerData::RemoveDepot(const BWAPI::Unit unit) {
  if (!unit) { return; }

  depots_.erase(unit);
  depot_2_worker_count_.erase(unit);
...
void WorkerData::AddWorkerNumOfMineralPatch(BWAPI::Unit unit, int num) {
  if (mineral_patch_2_worker_num_.find(unit) == mineral_patch_2_worker_num_.end()) {
    mineral_patch_2_worker_num_[unit] = num;
  }
  else {
    mineral_patch_2_worker_num_[unit] += num;
  }
...
void WorkerData::SetWorkerJob(const BWAPI::Unit unit, const WorkerJob job, const BWAPI::Unit job_unit) {
  ...
  if (job == kMinerals) {
    ...
    depot_2_worker_count_[job_unit] += 1;
    ...
    refinery_2_worker_count_[job_unit] += 1;
...
void WorkerData::ClearPreviousJob(const BWAPI::Unit unit) {
  ...
  if (previous_job == kMinerals) {
    depot_2_worker_count_[worker_2_depot_[unit]] -= 1;
    ...
    refinery_2_worker_count_[worker_2_refinery_[unit]] -= 1;
...
bool WorkerData::IsDepotFull(const BWAPI::Unit depot) {
  ...
  const int assignedWorkers = getNumAssignedWorkers(depot);
...
BWAPI::Unit WorkerData::GetWorkerResource(BWAPI::Unit unit) {
  ...
  if (GetWorkerJob(unit) == kMinerals) {
    it = worker_2_mineral_patch_.find(unit);
    if (it != worker_2_mineral_patch_.end()) {
      return it->second;
    }
  }
  else if (GetWorkerJob(unit) == kGas) {
    it = worker_2_refinery_.find(unit);
...
int WorkerData::getNumAssignedWorkers(BWAPI::Unit unit) {
  ...
  if (unit->getType().isResourceDepot()) {
    it = depot_2_worker_count_.find(unit);

    // if there is an entry, return it
    if (it != depot_2_worker_count_.end()) {
      return it->second;
    }
  }
  else if (unit->getType().isRefinery()) {
    it = refinery_2_worker_count_.find(unit);

    // if there is an entry, return it
    if (it != refinery_2_worker_count_.end()) {
      return it->second;
    }
      // otherwise, we are only calling this on completed refineries, so set it
    else {
      refinery_2_worker_count_[unit] = 0;
    }
  }
...
std::set<BWAPI::Unit> WorkerData::GetGasWorkers() {
