...
void ScoutManager::moveGroundScout() {
  ...
  if (_workerScoutTarget.isValid()) {
    ...
    the.micro_.Move(_workerScout, BWAPI::Position(_workerScoutTarget));
  }
  else {
    ...
    int scoutDistanceToEnemy = MapTools::Instance().getGroundTileDistance(
      _workerScout->getPosition(), enemyBaseLocation->getPosition());
    ...
    if (scoutInRangeOfenemy) {
      if (_scoutUnderAttack) {
        ...
        if (Config::Strategy::ScoutHarassEnemy && closestWorker && !wantGasSteal()) {
          ...
          _currentRegionVertexIndex = -1;
          the.micro_.CatchAndAttackUnit(_workerScout, closestWorker);
        }
...
void ScoutManager::moveAirScout() {
  ...
  if (enemyBaseLocation) {
    ...
    if (!_overlordAtEnemyBase) {
      ...
      the.micro_.Move(_overlordScout, enemyBaseLocation->getPosition());
    ...
    if (_overlordScoutTarget.isValid()) {
      the.micro_.Move(_overlordScout, BWAPI::Position(_overlordScoutTarget));
    }
  }
}
...
void ScoutManager::followPerimeter() {
  ...
  the.micro_.Move(_workerScout, fleeTo);
}
...
bool ScoutManager::gasSteal() {
  ...
  if (_enemyGeyser->isVisible() && _enemyGeyser->getType() != BWAPI::UnitTypes::Resource_Vespene_Geyser) {
    ...
    if (!ProductionManager::Instance().isGasStealInQueue()) {
      ...
      the.micro_.Move(_workerScout, _enemyGeyser->getInitialPosition());
    }
    _gasStealStatus = "Stealing gas";
  }
  else {
...
BWAPI::Position ScoutManager::getFleePosition() {
  ...
  double distanceFromCurrentVertex = _enemyRegionVertices[_currentRegionVertexIndex].getDistance(
    _workerScout->getPosition());
  ...
  while (distanceFromCurrentVertex < 128) {
    ...
    distanceFromCurrentVertex = _enemyRegionVertices[_currentRegionVertexIndex].
      getDistance(_workerScout->getPosition());
  }
...
void ScoutManager::calculateEnemyRegionVertices() {
  ...
  for (size_t i(0); i < closestTobase.size(); ++i) {
    ...
    bool edge =
      BWTA::getRegion(BWAPI::TilePosition(tp.x + 1, tp.y)) != enemyRegion || !BWAPI::Broodwar->isBuildable(
        BWAPI::TilePosition(tp.x + 1, tp.y))
      || BWTA::getRegion(BWAPI::TilePosition(tp.x, tp.y + 1)) != enemyRegion || !BWAPI::Broodwar->isBuildable(
        BWAPI::TilePosition(tp.x, tp.y + 1))
      || BWTA::getRegion(BWAPI::TilePosition(tp.x - 1, tp.y)) != enemyRegion || !BWAPI::Broodwar->isBuildable(
        BWAPI::TilePosition(tp.x - 1, tp.y))
      || BWTA::getRegion(BWAPI::TilePosition(tp.x, tp.y - 1)) != enemyRegion || !BWAPI::Broodwar->isBuildable(
        BWAPI::TilePosition(tp.x, tp.y - 1));
    ...
    if (edge && BWAPI::Broodwar->isBuildable(tp)) {
      if (Config::Debug::DrawScoutInfo) {
        ...
        BWAPI::Broodwar->drawTextMap(x1 + 3, y1 + 2, "%d",
                                     MapTools::Instance().getGroundTileDistance(BWAPI::Position(tp), basePosition));
