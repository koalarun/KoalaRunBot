...
{
	const BWAPI::Unitset tanks = Intersection(getUnits(), cluster.units_);
	if (tanks.empty())
	{
		return;
	}
		...
		if (order.isCombatOrder())
		{
			if (!tankTargets.empty())
			{
				...
				if (target)
				{
					if (tank->isSieged())
					{
						the.micro_.AttackUnit(tank, target);
					}
					else
					{
						the.micro_.KiteTarget(tank, target);
					}
				}
 			}
			...
			else
			{
				...
				if (tank->getDistance(order.getPosition()) > 100)
				{
                    ...
                    {
    					// move to it
    					the.micro_.AttackMove(tank, order.getPosition());
                    }
				}
			}
		}
