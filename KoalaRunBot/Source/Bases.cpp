...
bool Bases::checkIslandMap() const
{
	...
	int ourPartition = the.map_partitions_.id(startingBase->getTilePosition());

	for (BWAPI::TilePosition pos : BWAPI::Broodwar->getStartLocations())
	{
		// Is any other start position reachable from here by ground?
		if (pos != startingBase->getTilePosition() && ourPartition == the.map_partitions_.id(pos))
		{
			return false;
		}
	}
...
void Bases::drawBaseInfo() const
{
	...
	for (const auto & rejected : nonbases)
	{
		...
		{
			BWAPI::Broodwar->drawTextMap(resource->getInitialPosition() + BWAPI::Position(-16, -6), "%c%d", kRed, i);
...
void Bases::drawBaseOwnership(int x, int y) const
{
	...
	BWAPI::Broodwar->drawTextScreen(x, yy, "%cBases", kWhite);
	...
	for (Base * base : bases)
	{
		...
		BWAPI::Broodwar->drawTextScreen(x - 8, yy, "%c%c", kWhite, reservedChar);
...
{
	return the.map_partitions_.id(pos) == the.map_partitions_.id(startingBase->getTilePosition());
}
...
{
	return the.map_partitions_.id(tile) == the.map_partitions_.id(startingBase->getTilePosition());
}
