...
KoalaRunBot::FastApproximation fap;
...
namespace KoalaRunBot {
  ...
  void FastApproximation::simulate(int n_frames) {
    while (n_frames--) {
  ...
  std::pair<std::vector<FastApproximation::FAPUnit>*, std::vector<FastApproximation::FAPUnit>*> FastApproximation::
  getState() {
  ...
  void FastApproximation::dealDamage(const FastApproximation::FAPUnit& fu, int damage,
  ...
  int inline FastApproximation::DistButNotReally(const FastApproximation::FAPUnit& u1,
                                                 const FastApproximation::FAPUnit& u2) {
  ...
  void FastApproximation::UnitSim(const FastApproximation::FAPUnit& fu,
    ...
    auto closest_enemy = enemyUnits.end();
    ...
    for (auto enemyIt = enemyUnits.begin(); enemyIt != enemyUnits.end(); ++ enemyIt) {
      if (enemyIt->flying) {
        if (fu.airDamage) {
          ...
          if ((closest_enemy == enemyUnits.end() || d < closest_dist) && d >= fu.airMinRange) {
            closest_dist = d;
            closest_enemy = enemyIt;
          }
        }
      }
      else {
        if (fu.groundDamage && (!enemyIt->underSwarm || hitUnderSwarm)) {
          int d = DistButNotReally(fu, *enemyIt);
          if ((closest_enemy == enemyUnits.end() || d < closest_dist) && d >= fu.groundMinRange) {
            closest_dist = d;
            closest_enemy = enemyIt;
          }
        }
    ...
    if (closest_enemy != enemyUnits.end() && sqrt(closest_dist) <= fu.speed && !(fu.x == closest_enemy->x && fu.y ==
      closest_enemy->y)) {
      fu.x = closest_enemy->x;
      fu.y = closest_enemy->y;
      closest_dist = 0;

      didSomething = true;
    }

    if (closest_enemy != enemyUnits.end() && closest_dist <= (closest_enemy->flying ? fu.groundMaxRange : fu.airMinRange)) {
      if (closest_enemy->flying)
        dealDamage(*closest_enemy, fu.airDamage, fu.airDamageType), fu.attackCooldownRemaining = fu.airCooldown;
      else {
        dealDamage(*closest_enemy, fu.groundDamage, fu.groundDamageType);
        fu.attackCooldownRemaining = fu.groundCooldown;
        if (fu.elevation != -1 && closest_enemy->elevation != -1)
          if (closest_enemy->elevation > fu.elevation)
            fu.attackCooldownRemaining += fu.groundCooldown;
      }

      if (closest_enemy->health < 1) {
        auto temp = *closest_enemy;
        *closest_enemy = enemyUnits.back();
        enemyUnits.pop_back();
    ...
    else if (closest_enemy != enemyUnits.end() && sqrt(closest_dist) > fu.speed) {
      int dx = closest_enemy->x - fu.x, dy = closest_enemy->y - fu.y;
  ...
  void FastApproximation::Medicsim(const FAPUnit& fu, std::vector<FAPUnit>& friendlyUnits) {
    auto closest_heal_able = friendlyUnits.end();
    int closest_dist = 99999;
    ...
    for (auto it = friendlyUnits.begin(); it != friendlyUnits.end(); ++it) {
      if (it->isOrganic && it->health < it->maxHealth && !it->didHealThisFrame) {
        ...
        if (closest_heal_able == friendlyUnits.end() || d < closest_dist) {
          closest_heal_able = it;
          closest_dist = d;
        }
      }
    }

    if (closest_heal_able != friendlyUnits.end()) {
      fu.x = closest_heal_able->x;
      fu.y = closest_heal_able->y;
      ...
      closest_heal_able->health += (closest_heal_able->healTimer += 300) / 256;
      closest_heal_able->healTimer %= 256;

      if (closest_heal_able->health > closest_heal_able->maxHealth)
        closest_heal_able->health = closest_heal_able->maxHealth;

      closest_heal_able->didHealThisFrame = false;
    }
  }
  ...
  bool FastApproximation::suicideSim(const FAPUnit& fu, std::vector<FAPUnit>& enemyUnits) {
    ...
    if (closestEnemy != enemyUnits.end() && sqrt(closestDist) <= fu.speed) {
      ...
      const auto dx = closestEnemy->x - fu.x, dy = closestEnemy->y - fu.y;
  ...
  FastApproximation::FAPUnit::FAPUnit(UnitInfo ui) :
    ...
    groundCooldown(ui.type.groundWeapon().damageFactor() && ui.type.maxGroundHits()
                     ? ui.player->weaponDamageCooldown(ui.type) / (ui.type.groundWeapon().damageFactor() * ui
                                                                                                           .type.
                                                                                                           maxGroundHits()
                     )
                     : 0),
    ...
    airCooldown(ui.type.airWeapon().damageFactor() && ui.type.maxAirHits()
                  ? ui.type.airWeapon().damageCooldown() / (ui.type.airWeapon().damageFactor() * ui.type.maxAirHits())
                  : 0),
  ...
  const FastApproximation::FAPUnit& FastApproximation::FAPUnit::operator=(const FAPUnit& other) const {
    ...
    speed = other.speed, armor = other.armor, flying = other.flying, underSwarm = other.underSwarm, unitSize = other.
      unitSize;
    groundDamage = other.groundDamage, groundCooldown = other.groundCooldown, groundMaxRange = other.groundMaxRange,
      groundMinRange = other.groundMinRange, groundDamageType = other.groundDamageType;
    airDamage = other.airDamage, airCooldown = other.airCooldown, airMaxRange = other.airMaxRange, airMinRange = other.
      airMinRange, airDamageType = other.airDamageType;
    score = other.score;
    attackCooldownRemaining = other.attackCooldownRemaining;
    unitType = other.unitType;
    isOrganic = other.isOrganic;
    healTimer = other.healTimer;
