...
void CombatCommander::initializeSquads() {
  ...
  SquadOrder idleOrder(SquadOrderTypes::Idle, BWAPI::Position(BWAPI::Broodwar->self()->getStartLocation()), 100,
                       "Chill out");
  ...
  if (Config::Micro::ScoutDefenseRadius > 0) {
    SquadOrder enemyScoutDefense(SquadOrderTypes::Defend, ourBasePosition, Config::Micro::ScoutDefenseRadius,
                                 "Get the scout");
...
void CombatCommander::updateBaseDefenseSquads() {
  ...
  for (Base* base : Bases::Instance().getBases()) {
    ...
    int numEnemyFlyingInRegion = std::count_if(enemyUnitsInRegion.begin(), enemyUnitsInRegion.end(),
...
void CombatCommander::updateDefenseSquadUnits(Squad& defenseSquad, const size_t& flyingDefendersNeeded,
                                              const size_t& groundDefendersNeeded, bool pullWorkers,
...
BWAPI::Unit CombatCommander::findClosestDefender(const Squad& defenseSquad, BWAPI::Position pos, bool flyingDefender,
                                                 bool pullWorkers, bool enemyHasAntiAir) {
...
void CombatCommander::doComsatScan() {
  ...
  for (const auto unit : BWAPI::Broodwar->enemy()->getUnits()) {
    if (unit->isVisible() &&
      ...
      (void)the.micro_.Scan(unit->getPosition());
    ...
    */
    ...
    for (Base* base : Bases::Instance().getBases()) {
      if (base->getBlockers().size() > 0 &&
        ...
        squadPartition == the.map_partitions_.id(base->getPosition())) {
...
BWAPI::Position CombatCommander::getAttackLocation(const Squad* squad) {
  ...
  const int squadPartition = the.map_partitions_.id(Bases::Instance().myStartingBase()->getTilePosition());
  ...
  if (canAttackGround) {
    ...
    for (Base* base : Bases::Instance().getBases()) {
      if (base->getOwner() == BWAPI::Broodwar->enemy()) {
        ...
        if (hasGround && squadPartition != the.map_partitions_.id(base->getTilePosition())) {
          continue;
        }
  ...
  if (canAttackGround || BWAPI::Broodwar->enemy()->getRace() == BWAPI::Races::Terran) {
    for (const auto& kv : InformationManager::Instance().getUnitInfo(BWAPI::Broodwar->enemy())) {
      ...
      if (ui.type.isBuilding() &&
        ...
        (!hasGround || (!ui.type.isRefinery() && squadPartition == the.map_partitions_.id(ui.lastPosition)))) {
  ...
  for (const auto unit : BWAPI::Broodwar->enemy()->getUnits()) {
    ...
    if (hasGround &&
      squadPartition != the.map_partitions_.id(unit->getPosition()) &&
