...
void OpponentModel::considerGasSteal()
{
	...
	double plainUCB = plainWinRate + Ucb1Bound(plainGames, nGames);
	double stealUCB = stealWinRate + Ucb1Bound(nStealTries, nGames);
...
void OpponentModel::update()
{
	...
	if (Config::IO::ReadOpponentModel || Config::IO::WriteOpponentModel)
	{
		...
		if (_bestMatch)
		{
			...
			BWAPI::Broodwar->drawTextScreen(220, 6, "%cmatch", kWhite);
		}
		else
		{
			BWAPI::Broodwar->drawTextScreen(220, 6, "%cno best match", kWhite);
		}
	}
}
