...
void UnitStatistic::updateGoneFromLastPosition()
{
	for (auto & kv : unitMap)
	{
...
void UnitStatistic::updateUnit(BWAPI::Unit unit)
{
	if (!unit) { return; }
...
void UnitStatistic::removeUnit(BWAPI::Unit unit)
{
	if (!unit) { return; }
...
void UnitStatistic::removeBadUnits()
{
...
const bool UnitStatistic::badUnitInfo(const UnitInfo & ui) const
{
    if (!ui.unit)
    {
        return false;
...
int UnitStatistic::getGasLost() const 
{ 
    return gasLost; 
}

int UnitStatistic::getMineralsLost() const 
{ 
    return mineralsLost; 
}

int UnitStatistic::getNumUnits(BWAPI::UnitType t) const 
{ 
    return numUnits[t.getID()]; 
}

int UnitStatistic::getNumDeadUnits(BWAPI::UnitType t) const 
{ 
    return numDeadUnits[t.getID()]; 
}

const std::map<BWAPI::Unit,UnitInfo> & UnitStatistic::getUnits() const 
{ 
    return unitMap; 
}