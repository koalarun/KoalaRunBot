...
void ProductionManager::drawProductionInformation(int x, int y)
{
	...
	{
		y += 10;
		BWAPI::Broodwar->drawTextScreen(x, y, " %cgoal %c%s", kWhite, kOrange, NiceMacroActName(goal.act.getName()).c_str());
	}
	...
	for (const auto & unit : prod)
    {
		...
		BWAPI::Broodwar->drawTextScreen(x, y, " %c%s", kGreen, NiceMacroActName(t.getName()).c_str());
		BWAPI::Broodwar->drawTextScreen(x - 35, y, "%c%6d", kGreen, unit->getRemainingBuildTime());
	}
