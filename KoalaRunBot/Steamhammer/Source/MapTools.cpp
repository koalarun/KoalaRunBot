...
void MapTools::drawHomeDistances()
{
    ...
    for (int x = 0; x < BWAPI::Broodwar->mapWidth(); ++x)
    {
        ...
        {
			int dist = d.at(x, y);
			char color = dist == -1 ? kOrange : kWhite;
...
Base * MapTools::nextExpansion(bool hidden, bool wantMinerals, bool wantGas)
{
    ...
    for (Base * base : Bases::Instance().getBases())
    {
		...
		if (distanceFromUs < 0)
        {
			if (the.map_partitions_.id(tile) == the.map_partitions_.id(myBasePosition))
			{
		...
		if (enemyBase) {
			...
			if (distanceFromEnemy < 0)
			{
				...
				if (the.map_partitions_.id(tile) == the.map_partitions_.id(enemyTile))
				{
