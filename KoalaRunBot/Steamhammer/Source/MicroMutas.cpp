...
void MicroMutas::attackAssignedTargets(const BWAPI::Position & center)
{
	...
	if (assignments.empty())
	{
		...
		for (BWAPI::Unit muta : getUnits())
		{
			...
			{
				the.micro_.Move(muta, order.getPosition());
			}
			else
			{
				the.micro_.Move(muta, center);
	...
	else
	{
		...
		for (std::pair<BWAPI::Unit, BWAPI::Unit> assignment : assignments)
		{
			...
			BWAPI::Broodwar->drawTextMap(target->getPosition() + BWAPI::Position(-6, 6), "%c%d", kWhite, hitsToKill(target));
			if (targetsHaveAntiAir)
			{
				the.micro_.MutaDanceTarget(muta, target);
			}
			else
			{
				the.micro_.CatchAndAttackUnit(muta, target);
			}
		}
	}
}
...
void MicroMutas::executeMicro(const BWAPI::Unitset & targets, const UnitCluster & cluster)
{
	...
	for (BWAPI::Unit muta : getUnits())
	{
		if (muta->isIrradiated())
		{
			...
			{
				the.micro_.Move(muta, getFleePosition(muta, center, targets));
			}
			else
			{
				the.micro_.AttackMove(muta, getFleePosition(muta, center, targets));
			}
		}
