...
void SquadData::drawSquadInformation(int x, int y) 
{
	...
	for (const auto & kv : _squads) 
	{
		...
		BWAPI::Broodwar->drawTextScreen(x      , y + (yspace * 10), "%c%c%c", kCyan, visibibleOnly, meatgrinder);
		BWAPI::Broodwar->drawTextScreen(x +  20, y + (yspace * 10), "%c%c", kYellow, code);
		BWAPI::Broodwar->drawTextScreen(x +  30, y + (yspace * 10), "%c%s", kWhite, squad.getName().c_str());
		BWAPI::Broodwar->drawTextScreen(x + 100, y + (yspace * 10), "%c%d", kYellow, units.size());
		BWAPI::Broodwar->drawTextScreen(x + 125, y + (yspace * 10), "%c%d,%d", kYellow, orderTile.x, orderTile.y);
		++yspace;
