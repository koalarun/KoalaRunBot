...
{
	const BWAPI::Unitset & lurkers = Intersection (getUnits(), cluster.units_);
	if (lurkers.empty())
	{
		return;
	}
	...
	for (const auto lurker : lurkers)
	{
		...
		if (target)
		{
			...
			if (Config::Debug::DrawUnitTargetInfo) {
				...
				BWAPI::Broodwar->drawTextMap(lurker->getPosition() + BWAPI::Position(20, -10), "%c%d/192", kWhite, dist);
			}
				...
				cannonDanger && dist > cannonRange + 32 && dist < cannonRange + 128)
			{
				...
				else if (lurker->isBurrowed())
				{
					if (dist <= lurkerRange)
					{
						the.micro_.AttackUnit(lurker, target);
					}
				}
			}
				...
				isThreat && dist > std::max(lurkerRange, 32 + UnitUtil::GetAttackRange(target, lurker)))
			{
				...
				{
					the.micro_.Move(lurker, target->getPosition());
				}
			}
			else
			{
		...
		else
		{
			...
			else
			{
				...
				{
					the.micro_.Move(lurker, order.getPosition());
				}
			}
		}
	}
