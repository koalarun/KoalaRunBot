...
const BuildOrder& StrategyManager::getOpeningBookBuildOrder() const {
  ...
  if (buildOrderIt != std::end(_strategies)) {
    ...
    UAB_ASSERT_WARNING(false, "Strategy not found: %s, returning empty initial build order", Config::Strategy::
      StrategyName.c_str());
...
const MetaPairVector StrategyManager::getTerranBuildOrderGoal() {
  ...
  if (_openingGroup == "anti-rush") {
    ...
    const int enemiesCounteredByTanks =
      InformationManager::Instance().getNumUnits(BWAPI::UnitTypes::Terran_Siege_Tank_Tank_Mode,
                                                 BWAPI::Broodwar->enemy()) +
      InformationManager::Instance().getNumUnits(BWAPI::UnitTypes::Terran_Siege_Tank_Siege_Mode,
                                                 BWAPI::Broodwar->enemy()) +
  ...
  if (numCC > 0 && hasAcademy) {
    goal.push_back(std::pair<MacroAct, int>(BWAPI::UnitTypes::Terran_Comsat_Station,
                                            UnitUtil::GetCompletedUnitCount(BWAPI::UnitTypes::Terran_Command_Center)));
  }
...
void StrategyManager::handleUrgentProductionIssues(BuildOrderQueue& queue) {
  ...
  if (_selfRace == BWAPI::Races::Zerg) {
    ...
    if (BuildingManager::Instance().getStalledForLackOfSpace()) {
      if (_selfRace == BWAPI::Races::Protoss &&
        (!nextInQueuePtr || !nextInQueuePtr->isBuilding() || nextInQueuePtr->getUnitType() != BWAPI::UnitTypes::
          Protoss_Pylon) &&
