	...
	for (const auto rangedUnit : rangedUnits)
	{
		...
		if (rangedUnit->isIrradiated() && rangedUnit->getType().getRace() == BWAPI::Races::Zerg)
		{
			if (rangedUnit->isFlying())
			{
				...
				{
					the.micro_.AttackMove(rangedUnit, order.getPosition());
				}
				else
				{
					the.micro_.Move(rangedUnit, order.getPosition());
				}
				continue;
			}
		...
		if (stayHomeUntilReady(rangedUnit))
		{
			...
			the.micro_.AttackMove(rangedUnit, fleeTo);
			continue;
		}
		...
		if (order.isCombatOrder())
        {
			...
			if (target)
			{
				...
				{
					the.micro_.KiteTarget(rangedUnit, target);
				}
				else
				{
					the.micro_.CatchAndAttackUnit(rangedUnit, target);
				}
			}
			else
			{
