...
BOSSManager::BOSSManager()
  : _previousSearchStartFrame(0)
    ...
    , _previousStatusDesc("No Searches") {}
...
void BOSSManager::startNewSearch(const std::vector<MetaPair>& goalUnits) {
  ...
  if (numWorkers == 0) {
    _previousStatusDesc = "\x08No Workers :(";
    return;
  }

  if (numDepots == 0) {
    _previousStatusDesc = "\x08No Depots :(";
    return;
  }
  ...
  try {
    ...
    BOSS::GameState initialState(BWAPI::Broodwar, BWAPI::Broodwar->self(),
                                 BuildingManager::Instance().buildingsQueued());
...
void BOSSManager::drawSearchInformation(int x, int y) {
  ...
  int width = 155;
  int height = 80;
  BWAPI::Broodwar->drawBoxScreen(BWAPI::Position(x - 5, y), BWAPI::Position(x + width, y + height),
                                 BWAPI::Colors::Black, true);
  ...
  y += 10;
  BWAPI::Broodwar->drawTextScreen(BWAPI::Position(x, y), "%s", _previousStatusDesc.c_str());

  for (size_t i(0); i < _previousGoalUnits.size(); ++i) {
    if (_previousGoalUnits[i].second > 0) {
      y += 10;
      BWAPI::Broodwar->drawTextScreen(BWAPI::Position(x, y), "%d %s", _previousGoalUnits[i].second,
                                      _previousGoalUnits[i].first.getName().c_str());
    }
  }
  ...
  BWAPI::Broodwar->drawTextScreen(BWAPI::Position(x, y + 45), "BO Size: %d",
                                  (int)_savedSearchResults.buildOrder.size());
}
...
void BOSSManager::drawStateInformation(int x, int y) {
  ...
  BWAPI::Broodwar->drawTextScreen(BWAPI::Position(x - 100, y + 30), "\x04%s",
                                  currentState.getBuildingData().toString().c_str());
...
void BOSSManager::update(double timeLimit) {
  ...
  if (_smartSearch->getResults().solved) {
    if (_smartSearch->getResults().solutionFound) {
      _previousStatusDesc = std::string("\x07") + "BOSS Solve Solution\n";
    }
    else {
      _previousStatusDesc = std::string("\x03") + "BOSS Solve NoSolution\n";
    }
  }
  ...
  try {
    if (searchTimeOut) {
      _previousStatusDesc = std::string("\x02") + "BOSS Timeout\n";
    }

    if (caughtException) {
      _previousStatusDesc = std::string("\x02") + "BOSS Exception\n";
    }

    _previousBuildOrder = nbos.solve();
    _previousStatusDesc += "\x03NBOS Solution";
  }
  catch (const BOSS::BOSSException& exception) {
    // and if that search doesn't work then we're out of luck, no build orders for us
    _previousStatusDesc += "\x08Naive Exception";
    if (Config::Debug::DrawBuildOrderSearchInfo) {
